...
deconvEM <- function(Y,eta,mu,aber = TRUE, V = 'c', weight = matrix(1,5,5),
  ...
  for(iter in 1:maxiter){
    ...
    # E step
    pdf_a    = pi_a * dnorm(e/sqrt(Va)) / sqrt(Va)
    pdf_c    = (1 - pi_a)* dnorm(e/sqrt(Vc)) /sqrt(Vc)
  ...
  for(i in 1:folds){
    ...
    #  print('In cross validation')
      temp   <- deconvEM(Y_train,eta_train,mu_train,aber,V, weight = weight_train
                         ,pi_a_init,rho_init_train,nu0_init_train,
...
deconvEM_laplace <- function(Y,eta,mu,aber = TRUE, V = 'c', weight = matrix(1,5,5),
  ...
  for(iter in 1:maxiter){
    ...
    # E step
    pdf_a    = pi_a * dlaplace(e/Va) / Va
    pdf_c    = (1 - pi_a)* dlaplace(e/Vc)/Vc
